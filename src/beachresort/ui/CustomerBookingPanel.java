...
public class CustomerBookingPanel extends JPanel {
    ...
    // }
      ...
      private void setupLayout() {
        ...
        JButton editButton = createButton("Walk In", e -> showBookingDialogWalkIn(DialogMode.EDIT));
    ...
    private void showBookingDialog(DialogMode mode) {
     ...
     private void showBookingDialogWalkIn(DialogMode mode) {
        JDialog dialog = new JDialog();
        dialog.setTitle(getDialogTitle(mode));
        dialog.setSize(500, 600);
        dialog.setModal(true);
        ...
        // Form Components
        JPanel formPanel = createFormPanelWithAutoDates(mode);
        mainPanel.add(new JScrollPane(formPanel), BorderLayout.CENTER);
...
private JPanel createFormPanelWithAutoDates(DialogMode mode) {
    JPanel formPanel = new JPanel(new GridBagLayout());
    GridBagConstraints gbc = new GridBagConstraints();
    gbc.insets = new Insets(5, 5, 5, 5);
    gbc.anchor = GridBagConstraints.WEST;
    ...
    // Create text fields for other inputs
    for (int i = 0; i < labels.length; i++) {
        if (i != 3 && i != 4 && i != 5) {  // Skip room type, check-in, and check-out dates
            fields[i] = new JTextField(20);
        }
    }

    // Set Check-In Date to today and make it uneditable
    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    Date today = new Date();
    JTextField checkInField = new JTextField(dateFormat.format(today));
    checkInField.setEditable(false);
    fields[4] = checkInField;

    // Set Check-Out Date to tomorrow and make it uneditable
    Calendar calendar = Calendar.getInstance();
    calendar.setTime(today);
    calendar.add(Calendar.DATE, 1);
    JTextField checkOutField = new JTextField(dateFormat.format(calendar.getTime()));
    checkOutField.setEditable(false);
    fields[5] = checkOutField;
...
private void createNewBooking(JPanel formPanel) {
    ...
    private void createNewBooking1(JPanel formPanel) {
        Component[] components = formPanel.getComponents();
        ...
        if (bookingRepository.addBooking(newBooking)) {
            loadBookingsFromDatabase();
            JOptionPane.showMessageDialog(this, "Walk in recorded successfully!");
        } else {
            JOptionPane.showMessageDialog(this, "Failed to record walk in", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
